...
def nuDISxsection_NC_Tbl(Enu,neu):
    ...
    if Enu < 1.0e1 :
        ...
        if(neu == 0):
            ...
            sigNC = np.array([0.2492e-37,0.6033e-37,0.1391e-36,0.2261e-36,0.5430e-36,0.1255e-35,0.2039e-35,0.4781e-35,0.1035e-34,0.1575e-34,0.3139e-34,0.5615e-34,0.7667e-34,0.1280e-33,0.2017e-33,0.2600e-33,0.4018e-33,0.6001e-33,0.7482e-33,0.1104e-32,0.1581e-32,0.1939e-32,0.2763e-32,0.3837e-32,0.4641e-32,0.6490e-32,0.8931e-32,0.1066e-31,0.1465e-31,0.1995e-31,0.2377e-31,0.3247e-31,0.4377e-31,0.5196e-31])
...
def MuonMeanInelasticity_CC_Avg(E,neutype,datapath = global_datapath):
  ...
  if E >= 100.0 or E < 1.0:
	return MuonMeanInelasticity_CC_Tbl(E,neutype)
  else : 
	existfile = False 
	...
	if not existfile :       
	  ...
	  for i,EE in enumerate(Enu):
	    avg_y.append(MuonMeanInelasticity_CC_Tbl(EE,neutype)*ratio_DIS[i]+0.0*ratio_QE_RES[i])
	    avg_y_list.append([EE,MuonMeanInelasticity_CC_Tbl(EE,neutype)*ratio_DIS[i]+0.0*ratio_QE_RES[i]])
...
def MiniBooNECS(E,mbparam):
    """ Returns MiniBooNE CS for a given detection channel.
      ...
      Ref : A.A. Aguilar-Arevalo et al., "Measurement of nu_mu and anu_mu induced neutral current single \pi 0 production cross sections on mineral oil at E_nu O(1 GeV)", arXiv:0911.2063 [hep-ex]A.A. Aguilar-Arevalo et al., "Measurement of nu_mu and anu_mu induced neutral current single \pi_0 production cross sections on mineral oil at E_nu O(1 GeV)"
