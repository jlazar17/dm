...
try :
    # if need to disable the optneuosc interface comment the next line
    # and set OPTNEOSC_ERROR = True
    #import neutrinocommon.shareobjects.optneuosc as ono
    #OPTNEUOSC_ERROR = False
    OPTNEUOSC_ERROR = True
except ImportError:
    print "NC:NEU:NEUOSC:ERROR: Loading optimize neutrino oscillation functions : optneuosc."
    OPTNEUOSC_ERROR = True
try : 
    # if need to disable the tauola interface comment the next line
    # and set TAUOLA_ERROR = True. This would only work if you run : make taudecay
    ...
    TAUOLA_ERROR = True
try :
    # if need to disable the nusigma interface comment the next line
    # and set NUSIGMA_ERROR = True
...
class MixMatrix():
	...
	if(param.numneu == 2):
	    self.U = self.R(1,2,0,param)  
            ...
            # change the position of the CP phase to agree with SQuIDS parametrization
            self.U =  np.dot(self.R(3,4,0,param),np.dot(self.R(2,4,0,param),np.dot(self.R(1,4,2,param),np.dot(self.R(2,3,0,param),np.dot(self.R(1,3,1,param),self.R(1,2,0,param))))))
        elif(param.numneu == 5):
...
def flavorAcc(param,E,body,track):
    ...
    been factorized an is on the hamiltonian, but its keeped in case any new physics
    energy dependand interaction term need to be added.
...
def flavorAbsorption(param,E,body,track):
    """ Neutrino attenuation due to charge and neutral current interaction.
    
    In this aproximation NC and CC interaction are treated in the same way, i.e.
    the neutrino gets absorped. A full monte carlo is implemented in the GSL routine.
    ...
    Abs = np.zeros([param.numneu,param.numneu],complex)
    
    # electron fraction
    ye = body.ye(track)
    
    # Np = Ne : proton number, Nn : neutron number
    Np = param.Na*param.cm**-3*body.density(track)*ye
    Nn = Np*((1.0-ye)/ye)
    
    # cross sections. At high energies sig_nup = sig_nun
    # This cross sections are from NUSIGMA
    E_GeV = E/param.GeV
    ...
    # calculating absorption for active flavors
    Gamma_e = (Nn+Np)*(sig_NC+sig_CC_e)
    Gamma_mu = (Nn+Np)*(sig_NC+sig_CC_m)
    Gamma_tau = (Nn+Np)*(sig_NC+sig_CC_t)
    
    # calculating abs matrix
    Abs[0,0] = complex(Gamma_e,0.0)
    Abs[1,1] = complex(Gamma_mu,0.0)
    Abs[2,2] = complex(Gamma_tau,0.0)        
    
    return Abs
    
def flavorH(track,param,flavorM2,body,E,NCCC_int = False):
    ...
    @param  NCCC_int   :   If True will include attenuation effects.
    ...
    if not NCCC_int:
        ...
        return (ii)*(flavorM2/(2.0*E)+flavorAcc(param,E,body,track)+flavorAbsorption(param,E,body,track))

def RHS(neuvec,x,track,param,flavorM2,body,E,NCCC_int = False):
    ...
    rhs = np.dot(flavorH(track,param,flavorM2,body,E,NCCC_int = NCCC_int),neuvec)
    # Return to the real plane
    tmprhs = (np.array([[val[0].real,val[0].imag] for val in rhs])).flatten()       
    
    return tmprhs
    
def RHS_GSL(x,neuvec,params):
    """ Evaluates the RHS of Schrodinger's equation in the
    flavor basis
...
def RHS_INT_GSL(x,neuvec,params):
    """ Evaluates the RHS of Schrodinger's equation in
    the instantaneos interaction basis.
...
def EvolveNeuState(ineu,E,track,body,flavorM2,param,NCCC_int = False):
    ...
    if not OPTNEUOSC_ERROR : 
        fneu = cvec(integrate.odeint(ono.RHS,rvec(ineu.vec(param),param),[track.xini,track.xend],args = (track,param,flavorM2,body,E,NCCC_int),mxstep = 30000000,atol = 1.0e-8)[1],param)
    else :
        fneu = cvec(integrate.odeint(RHS,rvec(ineu.vec(param),param),[track.xini,track.xend],args = (track,param,flavorM2,body,E,NCCC_int),mxstep = 30000000,atol = 1.0e-8)[1],param) 
    return fneu    

def CalNeuOscPy(ineu,E,track,body,flavorM2,param,complete_decoherence = False, NCCC_int = False):
    """ Calculates the probability vector [P_ie,P_imu,P_itau,P_is,...] where i is 
    the neutrino flavor specified by ineu.
    
    UPDATE : Added the NCCC_int flag to include attenuation effects. Normally attenuation effects
    can be decoupled from oscillation effects due to the fact that cross sections are almost flavor
    indepent, but when sterile neutrinos are considered this is no longer the case. In this aproximation
    NC interactions are treated also like losses. 
    ...
    frkneu = EvolveNeuState(ineu,E,track,body,flavorM2,param,NCCC_int = NCCC_int)
...
def CalNeuOscGSL(ineu,E,track,body,flavorM2,param,file = None,family = 0,NCCC_int = False,complete_decoherence = False,survival_prob = False,E_threshold = 1.0,optimization = True,only_osc_prob = True,step_osc_prob = False,abs_error = 1.0e-7, rel_error = 1.0e-6):
    """ Calculates the probability vector [P_ie,P_imu,P_itau,P_is,...] where i is 
    ...
    @type   step_osc_prob:   boolean
    @param  step_osc_prob:   will return the intermidiate oscillation probabilities
    ...
    if optimization :
        if not OPTNEUOSC_ERROR:
            ...
            step    = stepper(dim,RHS_GSL,None,args = params)
    ...
    while r < track.xend :
        ...
        if NCCC_int : 
            ...
            if not OPTNEUOSC_ERROR:
                curprob = map(float,OscProb(ono.cvec(neu_state,param),param))
            else :
                curprob = map(float,OscProb(cvec(neu_state,param),param))
            # curstate will have ineu,E,x,p
            curstate = curprob
            
            curstate.insert(0,r)
            curstate.insert(0,E_GeV)
            curstate.insert(0,neu) 
            curstate.insert(0,ineu)

            osc_prob_list.append(curstate)  
    ...
    if file == None:
	    ...
	    return osc_prob_list
	else :
            return result
    elif survival_prob:
        gt.hwritefile(file,survival_prob_list,param,header = False)
    elif step_osc_prob:
	gt.hwritefile(file,osc_prob_list,param,header = False)
    else :
...
def InterSurProb(E,param,Emin,Emax,datapath = "../data", interaction = 'NC', filename_append = '', return_interpolator = False):
    ...
    @param  interaction   :   NC: only NC interactions, CC: only CC interactions, NCCC : NC and CC interactions.
    ...
    if PC.pabs_n_inter == 0.0 or PC.pabs_a_inter == 0.0 :
        for neu in ["neutrino","antineutrino"]:
            if filename_append == "":
                filename = "DatNeu"+body.name+"AbsorptionProbability_int_"+interaction+"_"+str(Emin/param.GeV)+"_GeV_"+str(Emax/param.GeV)+"_GeV_"+str(param.neutype)+".dat"
            else :
                filename = "DatNeu"+body.name+"AbsorptionProbability_int_"+interaction+"_"+str(Emin/param.GeV)+"_GeV_"+str(Emax/param.GeV)+"_GeV_"+str(param.neutype)+"_"+str(filename_append)+".dat"
            #filename = "DatNeuSunAbsorptionProbability_"+str(Emin/pc.GeV)+"_GeV_"+str(Emax/pc.GeV)+"_GeV_"+neu+".dat"
...
def GenDataOscProbFlavor(ineu,Emin,Emax,param,body = bd.Vacuum(),track = None,savepath = "../data",binnum = 100, integrator = "GSL",complete_decoherence = False,filename_append = ''):
...
def GenDataSurProb(Emin,Emax,track,body,param = PC.PhysicsConstants(),savedata = True,datapath = "../data/",interaction="NC",filename_append = ''):
	""" Calculates and saves the neutrino survival probability in a given body
	this calculation ONLY holds true for STD3 neutrino model. Else should
	look at options on CalOscProbGSL.
	...
	@type   track   :   track
	@param  track   :   trayectory inside the body
	@type   body    :   body
	@param  body    :   body where the neutrino will propagate.
	@type   param   :   physicsconstants
	@param  param   :   set of physical parameters to be used.
	@type   datapath   :   string
	@param  datapath   :   location where probability will be stored.
	@type   interaction   :   string
	@param  interaction   :   NC: only NC interactions, CC: only CC interactions, NCCC : NC and CC interactions.
	@type   filename_append    :   string
	@param  filename_append    :   append string to file name.
	...
	if param.neutype == "neutrino":
		...
		print "NC:NEU:NEUOSC:ERROR: GenDataSurProb : wrong neutype."
		quit()

	E_range = gt.LogSpaceEnergies(Emin,Emax, binnum = 200)
	p_abs   = []

	# get density array for this trayectory
	interdensity = gt.GetDensityProfile(track,body,steps = 1000,return_inter = True)
	
	if interaction == "NC":
		xsection = lambda EE,nn : xs.nuDISxsection_NC_Tbl(EE/param.GeV,nn)
	elif interaction == "CC":
		xsection = lambda EE,nn : xs.nuDISxsection_CC_Tbl(EE/param.GeV,nn)
	elif interaction == "NCCC":
		xsection = lambda EE,nn : xs.nuDISxsection_CC_Tbl(EE/param.GeV,nn)+xs.nuDISxsection_NC_Tbl(EE/param.GeV,nn)
	else :
		print "NC:NEU:NEUOSC:ERROR: GenDataSurProb : wrong interaction mode."

	print "BEGIN CALCULATION"
	for E in E_range:
		integrant = integrate.quad(lambda r :
			(xsection(E,neu))*
			(float(interdensity(r))*(param.gr/param.cm**3)/(param.atomic_mass_unit*param.gr))
			,track.xini,track.xend, limit = 100)[0]
		p_abs.append([E,np.exp(-integrant)])
	...
	if savedata:
		...
		print "Files created. SAVED : " + datapath
...
def flavorS(L,track,param,flavorM2,body,E):
    ...
    for i in np.arange(0,param.numneu,1):
        S = S + (np.exp(ii*eigen[i]*L)/(3*eigen[i]**2+c1))*((eigen[i]**2+c1)*Id+eigen[i]*T+np.dot(T,T))
    return S

def NuEvolution(E,track,body,flavorM2,param, nstep = 100):
    """ Evolves a state along a trayectory described by track
    in a body, considering the body as divided by nstep
    constant density slabs.
    ...
    @param  param   :   location where the data will be saved
    @type   nstep   :   int
    @param  nstep   :   number of slabs 
    """
    
    xpos = np.linspace(track.xini,track.xend, nstep)
    step = xpos[1] - xpos[0]
...
def CalNeuOscSlabs(ineu,E,track,body,flavorM2,param,nstep = 100,complete_decoherence = False):
    """ Evolves a state along a trayectory described by track
    in a body, considering the body as divided by nstep
    constant density slabs.
    ...
    S = NuEvolution(E,track,body,flavorM2,param, nstep = nstep)
    
    # initial state  
    dim = int(2*param.numneu)
    psi = [0.0]*dim
    psi[ineu*2] = 1.0             # initial state
    ...
    if complete_decoherence :
        PMNS = MixMatrix(param)
        if not OPTNEUOSC_ERROR:
            return ono.CompleteDecoherenceOscProb(psi,PMNS,param)
        else :
            return CompleteDecoherenceOscProb(psi,PMNS,param)    
    else : 
        return map(float,OscProb(psi,param))
