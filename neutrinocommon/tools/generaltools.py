...
#                        p_dict.append([slot,getattr(param,slot)])
        saveprop = ['name','linestyle','markerstyle','colorstyle','savefilename',
                    ...
                    'th14','th24','th34','th15','th25','th35','th45','delta1',
                    'delta2','delta3','dm21sq','dm31sq','dm32sq','dm41sq','dm51sq']
...
def Log10SpacePoints(Emin,Emax,binnum = 30):
	"""Returns log space points.
...
def GetDensityProfile(Track,Body,scanscale = "linear",steps = 100,return_inter = False,return_path=False,return_ye=False):
    """ Returns the density profile of a given body for a given track
        in units of gr/cm^3
        ...
        @type Body  : body
        @param Body : body 
        @type scanscale : string
        @param scanscale : "linear","log"
        @type steps : integer
        @param steps : number of positions to be tabulated

        @rtype  : array
        @return : density [gr/cm^3] array
    """
    xcur = Track.x
    xini = Track.xini
    xend = Track.xend
    xstep = (xend-xini)/steps
    
    if scanscale == "linear":
        x = np.arange(xini,xend+xstep,xstep)
    elif scanscale == "log":
        x = gt.LogSpacePoints(xini,xend,steps)
    
    densities = []
    for xx in x :
        Track.x = xx
        densities.append(Body.density(Track))

    ye = []
    for xx in x :
        Track.x = xx
        ye.append(Body.ye(Track))
