...
- Modified 22/OCT/2012 by C.A. Arguelles
    + Added the sun as a function of the "impact parameter"
    for Atmospheric Solar neutrino analysis
"""
import numpy as np
...
class VariableDensity():
    ...
    def __init__(self,rhomin,rhomax,yye):
        ...
        self.rhomax = rhomax
        self.yye = yye
        self.bodyparams = [rhomin,rhomax,yye]
...
class Sun():
    ...
    #6.9598E+5
    SunRadius = 6.9598E+5 #694439.0       # [km]
    Radius    = 6.9598E+5 #694439.0       # [km]
...
class SunASnu():
    ...
    def density(self,track):
        ...
        r = np.sqrt( Rsun2 + xkm*xkm - 2*xkm*np.sqrt(Rsun2-bkm*bkm) )/Rsun
...
class Star():
    ...
    def __init__(self,Radius,InitialRadius,InitialDensity):
        ...
        self.bodyparams = [Radius,InitialRadius,InitialDensity]
...
class EarthRadial():
    ...
    def density(self,track):
        ...
        xkm = track.x/pc.km
        # patch to conv. L back to km
        return self.rdensity(xkm/self.EarthRadius)
...
class EarthWithCavity():
    ...
    def __init__(self,rho_cavity,ye_cavity,wx,wy,th_rot,deep_cavity,th_cavity):
        ...
        self.bodyparams = [rho_cavity,ye_cavity,wx,wy,th_rot,deep_cavity,th_cavity]
...
#        if self.isInCavity(track) :
    ...
    class track():
        def __init__(self,phi,th):
            ...
            self.xin = d*pc.km
            self.xout = (w+d)*pc.km
            self.trackparams = [phi,th]
            
class EarthAtm():
    # class setup for atmospheric neutrino oscillation probabilities.
    name = "EarthAtm"
    id = 8
    ...
    def __init__(self):
        """ Initializes EarthAtm object which is optimized for atmospheric
        oscillation on the Earth.
        """
    ...
    def density(self,track):
        ...
        if track.cosphi <= 0.0 :
            ...
            xkm = track.x/pc.km
            h = xkm*track.cosphi
            # need to check this numbers
            h0 = 25.0 # [km]
            return 1.05*np.exp(h/h0)
    ...
    class track():
        def __init__(self,phi):
            ...
            # Calculatin baseline
            if self.cosphi <= 0.0 :
                self.L = 2.0*Radius*np.abs(self.cosphi)
            else :
                # approximate
                self.L = AtmHeight/np.abs(self.cosphi)

            self.x = 0.0
            self.xini = 0.0
            self.xend = self.L
            self.trackparams = [phi]

class AGN():
    name = "AGN"
    id = 5

    def __init__(self,radius,rhomax,yye):
        self.radius = radius
        self.rhomax = rhomax # [gr/cm^3]
        self.yye = yye       # [dimensionless]
        self.bodyparams = [radius,rhomax,yye]
    ...
    def density(self,track):
        ...
        return self.rdensity(xparsec/radius)
