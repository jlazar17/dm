...
def MuonEnergy(E,neutype,param):
    ...
    """
    return (1.0-xs.MuonMeanInelasticity_CC_Avg(E/param.GeV,neutype))*E    
...
def MuonRange_SM(Emu,param,Emumin = 1.0e+9):
...
def NucleonNumber(E,param,Emumin = 1.0e+9):
    ...
    """
    vol_eff     = IC.Aeffmu_trigger_interpolated(E/param.GeV)*max(MuonRange_SM(E,param,Emumin),1.0e3)*param.meter**3
...
def IceCubeEventNumber(E_range,numu_flux,anumu_flux,param):
    T = 1.0*param.year
    rho_ice = 1.0*param.gr/param.cm**3
    mp = param.proton_mass
    NT = rho_ice/mp
    
    integrant = lambda Enu,Emu_0,Emu_fin : \
                RR(Emu_0,Emu_fin,param)*Aeff_mu(Emu_fin,costh_min,costh_max,param)*\
                ()
    
        
def Aeff_mu(Emu_fin,costh_min,costh_max,param):
    integral = integrate.quad(lambda costh : A(Emu_fin,param)*R(costh),
                              costh_min,costh_mmax)[0]
    return 2.0*np.pi*integral
 
def A(Emu_fin,param):
    Emu_fin_GeV = Emu_fin/param.GeV
